function GaborData = ExperimentGabor_staircase(GaborData, varargin)
...
if isempty(GaborData.model_observer)
    ...
    GaborData.lims = [0 0 settings.screenSize(3) settings.screenSize(4)];
...
if isequal(GaborData.stair_fn, @Staircase.contrast)
    fileName = fullfile(datadir, [subjectID '-GaborDataStairCaseContrast.mat']);
    fileNameQuit = fullfile(datadir, [subjectID '-GaborDataStairCaseContrastQuit.mat']);
elseif isequal(GaborData.stair_fn, @Staircase.ratio)
    fileName = fullfile(datadir, [subjectID '-GaborDataStairCaseRatio.mat']);
    fileNameQuit = fullfile(datadir, [subjectID '-GaborDataStairCaseRatioQuit.mat']);
elseif isequal(GaborData.stair_fn, @Staircase.noise)
    fileName = fullfile(datadir, [subjectID '-GaborDataStairCaseNoise.mat']);
    fileNameQuit = fullfile(datadir, [subjectID '-GaborDataStairCaseNoiseQuit.mat']);
else
...
if isempty(GaborData.model_observer)
    ...
    DrawFormattedText(wPtr, ...
        ['You are required to keep your eyes on the white cross. You will be shown two equally-distant patterns in the periphery. ' ...
        'At the end of the trial, you will have to report if the two patterns were same or different' ...
...
try
    while trial <= GaborData.trials_per_block * GaborData.blocks
        ...
        [image_array, frame_categories, checksum] = GaborStimulus_staircase(GaborData, trial);
        sz = size(frame_categories);
        GaborData.frame_categories(trial, sz) = frame_categories;
        ...
        if isempty(GaborData.model_observer)
            ...
            %             image_array_new = reshape(image_array,2,GaborData.stim_size,GaborData.stim_size);
            
            [I, tracker_info, broke_condition, quit] = trialStimuliGabor_staircase(GaborData, unit8(image_array), wPtr, tracker_info, settings);
