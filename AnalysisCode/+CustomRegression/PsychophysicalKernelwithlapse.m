cfunction [weights, postVal, errors, map_ridge, map_ar1, map_curvature] = ...
    PsychophysicalKernelwithlapse(data, responses, num_frames,  hpr_ridge, hpr_ar1, hpr_curvature, standardize, opt_type)
...
for i=1:n_gridpts
    ...
    [weights, postVal, errors] = do_fit(data, responses, num_frames, hpr_ridge(idx_ridge), hpr_ar1(idx_ar1), hpr_curvature(idx_curvature), opt_type);
...
function [optim_weights, postVal, errors] = do_fit(data, responses, frames, hpr_ridge, hpr_ar1, hpr_curvature,opt_type)
...
% options=optimset('Display', 'off', 'MaxIter', 10000, 'MaxFunEvals', 10000,'Algorithm', 'trust-region',);
% options = optimset('MaxIter', 1000000, 'MaxFunEvals', 1000000,'Display','off');

if strcmp(opt_type,'model')
    options=optimset('Display', 'off', 'MaxIter', 10000, 'MaxFunEvals', 10000);%,'Algorithm', 'trust-region');
