function [best_hprs, log_likelihoods] = xValidatePK_with_lapseSabyaOLD(data, responses, frames, hpr_ridge, hpr_ar1, hpr_curvature, standardize, folds)
...
%         hpr_ridge(iRidge), hpr_ar1(iAR1), hpr_curvature(iCurve), 0);
    [params, ~, ~, ~, ~, ~] = ...
    CustomRegression.PsychophysicalKernelwithlapseSabya(data(dataIdx, :), responses(dataIdx), frames,  ...
        hpr_ridge(iRidge), hpr_ar1(iAR1), hpr_curvature(iCurve), 0);
...
weights = exp(params(1:end-2));
lapse = 1e-4+(1-1e-4)*sigmoid(params(end));
% lapse = (params(end))^2;
logits = data * weights(:) + params(end-1);
