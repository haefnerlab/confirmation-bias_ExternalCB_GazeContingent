...
if phase == 0
    ...
    uniq_vals = unique(Data.sign_contrast);
...
elseif phase == -3
    ...
    % Count how often subject was correct at each contrast value.
    uniq_vals = unique(Data.sign_contrast);
    yvals = arrayfun(@(u) mean(Data.choice(Data.sign_contrast == u) == +1), uniq_vals);
    num_trials_at_vals = arrayfun(@(u) sum(Data.sign_contrast == u), uniq_vals);
    stderrs = arrayfun(@(u) std(Data.choice(Data.sign_contrast == u) == +1), uniq_vals) ./ sqrt(num_trials_at_vals); 
