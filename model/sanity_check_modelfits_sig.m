...
for i=1:numel(sigs_true)
    for j=1:numel(nsamps_true)
        for k=1:nreps
            ...
            plot(log10(sigs),sum(ll1),'color',[1,0.75,0.75],'linewidth',2);
...
for i=1:3
    for j=1:3
        subplot(3,3,3*(i-1)+j);
        plot(log10(sigs),mean(ll1s{i,j}),'r','linewidth',4);
...
for i=1:numel(sigs_true)
    for j=1:numel(nsamps_true)
        ...
        plot(log10(sigs),sum(ll1),'color',[0.75,0.75,1],'linewidth',2);
...
for i=1:3
    for j=1:3
        subplot(3,3,3*(i-1)+j);
        plot(log10(sigs),mean(ll1s{i,j}),'b','linewidth',4);
