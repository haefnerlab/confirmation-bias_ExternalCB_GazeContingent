function [chosen_locs,not_chosen_locs,final_choice]=model_loglikelihood(frame_signals,params,scale_normalize,is_sampling)
...
for tr=1:num_trials
    ...
    I_first(tr) = normrnd(frame_signals(tr,1),sigma_fovea);
    I_others(tr,:,:) = normrnd([frame_signals(tr,2:2:end);frame_signals(tr,3:2:end)], sigma_fovea);
    I_peri(tr,:,:) = normrnd([frame_signals(tr,2:2:end);frame_signals(tr,3:2:end)], sigma_peri);
    ...
    for i=1:num_frames-1
        ...
        log_odds_l1 = log(normpdf(I_others(tr,1,i),-1,sigma_fovea)*p_match + normpdf(I_others(tr,1,i),1,sigma_fovea)*(1-p_match)) - log(normpdf(I_others(tr,1,i),-1,sigma_fovea)*(1-p_match) + normpdf(I_others(tr,1,i),1,sigma_fovea)*p_match);
        log_odds_l2 = log(normpdf(I_others(tr,2,i),-1,sigma_fovea)*p_match + normpdf(I_others(tr,2,i),1,sigma_fovea)*(1-p_match)) - log(normpdf(I_others(tr,2,i),-1,sigma_fovea)*(1-p_match) + normpdf(I_others(tr,2,i),1,sigma_fovea)*p_match);
        log_odds_l1_peri = log(normpdf(I_peri(tr,1,i),-1,sigma_peri)*p_match + normpdf(I_peri(tr,1,i),1,sigma_peri)*(1-p_match)) - log(normpdf(I_peri(tr,1,i),-1,sigma_peri)*(1-p_match) + normpdf(I_peri(tr,1,i),1,sigma_peri)*p_match);
        log_odds_l2_peri = log(normpdf(I_peri(tr,2,i),-1,sigma_peri)*p_match + normpdf(I_peri(tr,2,i),1,sigma_peri)*(1-p_match)) - log(normpdf(I_peri(tr,2,i),-1,sigma_peri)*(1-p_match) + normpdf(I_peri(tr,2,i),1,sigma_peri)*p_match);
    ...
    log_odds_l1_peri = log(normpdf(I_peri(tr,1,end),-1,sigma_peri)*p_match + normpdf(I_peri(tr,1,end),1,sigma_peri)*(1-p_match)) - log(normpdf(I_peri(tr,1,end),-1,sigma_peri)*(1-p_match) + normpdf(I_peri(tr,1,end),1,sigma_peri)*p_match);
    log_odds_l2_peri = log(normpdf(I_peri(tr,2,end),-1,sigma_peri)*p_match + normpdf(I_peri(tr,2,end),1,sigma_peri)*(1-p_match)) - log(normpdf(I_peri(tr,2,end),-1,sigma_peri)*(1-p_match) + normpdf(I_peri(tr,2,end),1,sigma_peri)*p_match);
