function [chosen_locs,not_chosen_locs,final_choice,log_odds_accumulated_agg]=simulate_model_v4(frame_signals,params,scale_normalize,is_sampling,orientation_std_exp)
...
I_first = im_fov(tr,1);
I_others = [im_fov(tr,2:2:end);im_fov(tr,3:2:end)];
I_peri = [im_peri(tr,2:2:end);im_peri(tr,3:2:end)];
    ...
    for i=1:num_frames-1
        ...
        b1= compute_bas_v3(I_peri(1,i),sigmoid(-1*log_odds_accumulated(i)),p_match,sigma_peri^2,[num_samples_total_entropy,num_samples_noise_entropy],sd1);
        ...
        if rand()<lapse_rate_saccade
            ...
            if b1>b2
%             if sign(I_peri(1,i))==-1*sign(log_odds_accumulated(i))  && sign(I_peri(1,i))==-1*sign(I_peri(2,i))
