function [chosen_locs,not_chosen_locs,final_choice,log_odds_accumulated]=simulate_model_v3(frame_signals,params,scale_normalize,is_sampling,orientation_std_exp)
...
% log_odds_accumulated(tr,1) = logodds_model(I_first(tr),sigma_fovea.^2,p_match);
    ...
    log_odds_accumulated = logodds_model(I_first,sigma_fovea.^2,p_match);
...
%         table_ct_cl1_cl2_given_d(tr,:,:,:) = compute_table(I_peri(tr,1,i),I_peri(tr,2,i),sigma_peri,squeeze(prob_ct(tr,:,i+1)),p_match,log_odds_accumulated(tr,i));
        ...
        table_ct_cl1_cl2_given_d(tr,:,:,:) = compute_table(I_peri(1,i),I_peri(2,i),sigma_peri,[0.5,0.5],p_match,log_odds_accumulated(i));

        sd1=randi(1e8);
        
        
        [b1,~,~] = compute_score(squeeze(table_ct_cl1_cl2_given_d(tr,:,:,:)),l1_dim,num_samples_total_entropy,num_samples_noise_entropy,is_sampling,sd1);
        [b2,~,~] = compute_score(squeeze(table_ct_cl1_cl2_given_d(tr,:,:,:)),l2_dim,num_samples_total_entropy,num_samples_noise_entropy,is_sampling,sd1);
